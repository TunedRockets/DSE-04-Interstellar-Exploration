...
def load_ISOs(filename: str, plot: bool = False, time: float = None):
    ...
    for ob, d_time, H in tqdm(ISOs, desc="Plotting ISOs"):
        if random.uniform(0, len(ISOs)) < 20:
            try:
...
def get_ISO(T: float = 0, rm: float = 10, gen_type: str = '') -> list[tuple[Orbit, float, str]]:
    ...
    q, e, theta, inc, RAAN, arg_p = synthetic_population(T,
                                                         rm, n0, v_min, v_max, u_sun, v_sun, w_sun, sigma_vx, sigma_vy,
                                                         sigma_vz, va, vd, R_reff)
...
def _detection_time(ob: Orbit, absolute_magnitude: Callable[[float], float], sensitivity: float) -> float:
    ...
    if F_low > 0:  # same sign, bad
        raise ArithmeticError(
            f"min magnitude is {F_low}, which is still positive (should be negative), periapsis is: {ob.periapsis / AU} AU")
