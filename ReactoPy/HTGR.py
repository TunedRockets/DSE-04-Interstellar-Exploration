...
class Reactor:
    ...
    def __init__(
        ...
        enrichment=0.20,    # fraction of U235 in fuel (0–1) https://world-nuclear.org/information-library/nuclear-fuel-cycle/conversion-enrichment-and-fabrication/high-assay-low-enriched-uranium-haleu
        burnup=0.15,        # fraction of fuel burned at EoM
        lifetime_s = 315360000, # operational period before refuel (s); default = 10 year
        operating_pressure = 3e6, # Pa helium pressure
        power_density=4.4e6,  # W/m^3 volumetric power density (4–8 MW/m^3 typical for HTGR)
        uranium_pebble_fraction = 0.04, # from sheets
        allowable_stress = 12e6, # from ASME Table 1B NO6617
        ...
        self.fuel_kg           = 0.0   # kg of total fuel (including matrix)
    ...
    def size_fuel(self):
        ...
        self.fuel_kg          = self.u235_kg / self.enrichment /self.uranium_pebble_fraction     # total fuel mass (including matrix)
    ...
    # ----------------------------------------------------------------------------------------------------------
    def size_reflector(self, thickness_ratio=0.54):
        """
    ...
    def size_pressure_vessel(self):
        ...
        Material: SA-508 Grade 3 steel (typical for reactor vessels)
        or Alloy 800H for very high temperatures.
        """
    ...
    def size_all(self, print_true=True):
        ...
        if print_true:
            ...
            print()
            print(f"  -- Core barrel --")
            print(f"  Barrel thickness   : {barrel['barrel_thickness_m']:.2f} m")
            print(f"  Barrel mass    : {barrel['barrel_mass_kg']:.2f} kg")
            print()
            print(f"  -- Pressure vessel --")
            print(f"  Pressure vessel thickness   : {vessel['wall_thickness_m']:.2f} m")
...
def main():
    reactorquestionmark = Reactor(1050, 1273, 100000)
